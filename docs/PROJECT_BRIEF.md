...
- Install (uv-managed venv): `torch` from the **cu128 index** (Blackwell support), `unsloth`, `transformers`, `datasets`, `peft`, `trl`, `bitsandbytes`, `accelerate`, `vllm`, plus `pandas scipy`. `llamafactory` optional (secondary trainer). Qwen3.5 requires current transformers and vllm (>=0.19-class) and latest unsloth (Qwen3.5 family support confirmed May 2026).
...
- Queue overnight downloads at the end of setup: Qwen3.5-9B, Qwen3.5-27B, and Fanar-2-27B-Instruct (Qwen3-32B optional as fallback if disk allows).
...
4. A 10-step QLoRA run on Qwen3.5-27B completes at 8K context without OOM (batch 1, grad-accum 16, gradient checkpointing).
...
Slice 1: eval harness v0 (`eval/run.py` — dual-axis accuracy, derivation faithfulness, calibration/ECE, escalation P/R, schema validity). Slice 2: gold-case authoring workflow (Appendix A/B templates from UDS + coverage-matrix updater). Slice 3: base-model verification runner. Everything plan-first, reviewer-verified, per this machine's four-prompt workflow.
